...
-- employee_shift and timekeeper_project are tied to a specific
-- project/shift, not core identity — delete for EVERYONE, including the
-- kept employee, since their old shift/project assignment no longer
-- applies once they're moved to DEMO. Excluding the kept employee here
-- (like the tables below) left a dangling reference blocking shift
-- deletion in the next part.
DELETE FROM employee_shift;
DELETE FROM timekeeper_project;
